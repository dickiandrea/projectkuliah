...
if(Input::get('update') == "berikode"){				
	...
	join 
	(select max(z.nomer_contoh) as nomer from status as z inner join tborder as y where kode_analisis like 'K%')d
	join
	status as e
	on
	...
	$penomoran = "K.".$kode_analisis.".".($nomer+1)."-".$totaljumlah;
	...
	$tambah = $insert->tambah('sericontoh',array(    
	  ...
	  if($up == TRUE and $down == TRUE and $tambah == TRUE){
		...
		document.location='?page=lihat_permohonan_kimia'; </script><?php
	  }else{
		?><script language="JavaScript">alert("Gagal input kode order dan nomor contoh!");
		document.location='?page=lihat_permohonan_kimia'; 
		</script><?php
	  }
