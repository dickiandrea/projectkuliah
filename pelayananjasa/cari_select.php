...
error_reporting(E_ALL);
	   ...
	   if(isset($_GET['kode']))
	   {
				...
				?>
					<div class="title">
						...
						<h1>Balittanah<small> Balai Penelitian Tanah</small></h1>
						  <div class="btn-group" role="group" aria-label="...">	
							<form method="post" action="?page=update_chat">
								<input type="hidden" name="penerima"  value="<?php echo $syarat;?>">
								<input type="hidden" name="pengirim" id="kode_useraktif" value="<?php echo $pengirim;?>">
						...
						<h1><?php echo strtoupper($nama);?><small> (<?php echo $email;?>)</small></h1>
						  <div class="btn-group" role="group" aria-label="...">	
							<form method="post" action="?page=update_chat">
								<input type="hidden" name="penerima"  value="<?php echo $syarat;?>">
								<input type="hidden" name="pengirim" id="kode_useraktif" value="<?php echo $pengirim;?>">
								<input type="submit" onclick="return confirm('Apakah anda yakin ingin menghapus ini?')" name="hapuschatpeljas" class="btn btn-danger btn-sm" value="Hapus Chat">
							</form>
						  </div>
					 </div>   
				<?php
		  ...
		  <div class="write">					  
		   ...
		   if($pengirim == 0){
				$kata = "insert into chatadmin values ('', '$pengirim','$penerima','$isi', now(),'ya', 'administrator')";
		   }else{
			   $kata = "insert into chatadmin values ('', '$pengirim','$penerima','$isi', now(),'ya', 'peljas')";
		   }		   
		   $result = $mysqli->query($kata);
			echo "<p>".$kata."</p>";
