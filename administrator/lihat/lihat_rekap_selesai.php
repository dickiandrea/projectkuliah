	...
	<div class="x_panel">
	  ...
	  <div class="x_content" style="margin-top:20px;">		
		...
		<div class="col-md-12 col-sm-12 col-xs-12" style="margin-bottom: 10px;">
			<p>Filter  :
			<select class="selectpicker" data-live-search="true" name="filter_bulan" id="filter_bulan" required>
				<option value="">Pilih Bulan</option>
				<?php
					$data = "select distinct concat(year(tanggal_terima),'-', month(tanggal_terima)) as bulan from tborder ";					
					$result = $mysqli->query($data);
					while($hasil = $result->fetch_assoc()){
						echo "<option value='".$hasil['bulan']."'>".Convert::convert_tanggal($hasil['bulan'])."</option>";
					}
				?>
			</select>						
			<select class="selectpicker" data-live-search="true" name="filter_laporansp" id="filter_laporansp" required>
